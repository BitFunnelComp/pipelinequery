//Native NVM query process without cache, just thread buffer
#include<iostream>
#include<vector>
#include<stdint.h>
#include<stdio.h>
...
int threadCount = 0;
vector<vector<double>>query_Times_NVM;
...
struct and_query {
	uint64_t operator()(term_id_vec terms)const
	{
		...
		for (auto term : terms)
		{
			uint64_t length = List_offset[term + 1] - List_offset[term], offset = List_offset[term];
			uint64_t readoffset = ((uint64_t)(offset / DISK_BLOCK))*DISK_BLOCK;
			uint64_t readlength = ((uint64_t)(ceil((double)(length + offset - readoffset) / DISK_BLOCK)))*DISK_BLOCK;
			posix_memalign((void**)&list_data[tmpcount], DISK_BLOCK, readlength);
			auto tick = get_time_usecs();
			pread(IndexFile, list_data[tmpcount], readlength, readoffset);
			double elapsed = double(get_time_usecs() - tick);
			query_Times_NVM[threadID].push_back(elapsed);
			enum_type tmplist(Head_Data.data() + Head_offset[term], list_data[tmpcount] + offset - readoffset, num_docs);
...
struct topk_queue {
	...
	void test_write_topK(string str, int threadID)
	{
...
struct block_max_wand_query {
	...
	uint64_t operator()(term_id_vec terms) {
		...
		{
			uint64_t length = List_offset[term.first + 1] - List_offset[term.first], offset = List_offset[term.first];
			uint64_t readoffset = ((uint64_t)(offset / DISK_BLOCK))*DISK_BLOCK;
			uint64_t readlength = ((uint64_t)(ceil((double)(length + offset - readoffset) / DISK_BLOCK)))*DISK_BLOCK;
			posix_memalign((void**)&list_data[tmpcount], DISK_BLOCK, readlength);
			auto tick = get_time_usecs();
			...
			double elapsed = double(get_time_usecs() - tick);
			query_Times_NVM[threadID].push_back(elapsed);
			enum_type list(Head_Data.data() + Head_offset[term.first], list_data[tmpcount] + offset - readoffset, num_docs);
		...
		m_topk.finalize();
		//m_topk.test_write_topK("BMW",threadID);
...
struct wand_query {
	...
	uint64_t operator()(term_id_vec const& terms)
	{
		...
		m_topk.finalize();
		//m_topk.test_write_topK("WAND",threadID);
		for (unsigned i = 0; i<query_term_freqs.size(); i++)
			free(list_data[i]);
		return m_topk.topk().size();
	}
...
struct maxscore_query {
	...
	uint64_t operator()(term_id_vec const& terms)
	{
		...
		m_topk.finalize();
		//m_topk.test_write_topK("MAXSCORE",threadID);
		for (unsigned i = 0; i<query_term_freqs.size(); i++)
			free(list_data[i]);
		return m_topk.topk().size();
	}
...
struct ranked_and_query {
	...
	uint64_t operator()(term_id_vec terms)
	{
		...
		m_topk.finalize();
		//m_topk.test_write_topK("RANKAND",threadID);
		for (unsigned i = 0; i<query_term_freqs.size(); i++)
			free(list_data[i]);
		return m_topk.topk().size();
	}
...
void initial_data()
{
	...
	query_Times_NVM.resize(threadCount);
}
void print_statistics(vector<vector<double>>query_times, string querytype)
{
	double averageLantancy = 0, avgReadLat = 0;
	for (int i = 0; i < threadCount; i++)
	{
		double avg = std::accumulate(query_times[i].begin(), query_times[i].end(), double());
		averageLantancy += avg;
		avg = std::accumulate(query_Times_NVM[i].begin(), query_Times_NVM[i].end(), double());
		avgReadLat += avg;
	}
	cout << "-------------" << querytype << "-------------" << endl;
	cout << "AvgQueryLat=" << averageLantancy / queries.size() << endl;
	cout << "AvgReadLat=" << avgReadLat / queries.size() << endl;
}
...
#pragma omp parallel for num_threads(threadCount) schedule(dynamic)
			...
			double elapsed = double(get_time_usecs() - tick);
			cout << "Performed WAND query, sum time=" << elapsed << ", avg time throught put)=" << queries.size() / elapsed * 1000000 << endl;
		}
...
#pragma omp parallel for num_threads(threadCount) schedule(dynamic)
			...
			double elapsed = double(get_time_usecs() - tick);
			cout << "Performed RANKAND query, sum time=" << elapsed << ", avg time throught put)=" << elapsed / queries.size() << endl;
		}
...
#pragma omp parallel for num_threads(threadCount) schedule(dynamic)
			...
			double elapsed = double(get_time_usecs() - tick);
			cout << "Performed BMW query, sum time=" << elapsed << ", avg time throught put)=" << queries.size() / elapsed * 1000000 << endl;
		}
	}
	close(IndexFile);
